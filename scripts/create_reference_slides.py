#!/usr/bin/env python3
"""Batch-generate imagegen prompts for reference slides from a deck outline.

Reads an outline, extracts section titles, and generates one imagegen prompt
file per slide. Uses create_reference_slide.py for individual prompts.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--style-guidance",
        default="",
        help="Optional deck brief and presentation style guidance from the user prompt.",
    )
...
def extract_section_titles(outline, total):
    paragraphs = [p.strip() for p in re.split(r"\n\s*\n", outline.strip()) if p.strip()]
    titles = []
...
def infer_slide_count(outline):
    paragraphs = [p.strip() for p in re.split(r"\n\s*\n", outline.strip()) if p.strip()]
    return max(1, len(paragraphs) - 1)


def build_slide_prompt(index, total, outline, style_guidance, section_titles):
    ...
    if style_guidance.strip():
        style_block = (
            ...
            "as editable python-pptx primitives:\n"
    ...
    return (
        ...
        "choose a structure that can be rebuilt with editable python-pptx primitives.\n\n"
        f"{style_block}"
        f"Full outline:\n{outline}"
    )


def render_slide(script_path, index, total, outline, style_guidance, section_titles, output_path, prompt_output_path, reference_image=None):
    cmd = [
        sys.executable,
        str(script_path),
...
def write_prompt_index(prompt_paths, output_path):
    lines = [
        "# Reference Slide Imagegen Prompt Index",
        "",
        "Use Codex imagegen to generate the intended PNGs from these prompt files.",
        "These prompts are rebuild blueprints for editable python-pptx slides, not final slide assets.",
        "",
    ]
...
def safe_path_segment(value):
    if not value:
        return None
    segment = re.sub(r"[^A-Za-z0-9._-]+", "-", str(value).strip()).strip(".-")
...
def main():
    ...
    render_slide(single_script, 1, slide_count, outline, args.style_guidance, section_titles, first_output, first_prompt)
