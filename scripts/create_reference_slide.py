#!/usr/bin/env python3
"""Write a Codex imagegen prompt for a professional PPTX reference slide.

The generated image is a rebuild blueprint for an editable PowerPoint deck,
not final bitmap art.
"""
...
Optimize for layouts and visuals that can be reconstructed cleanly with python-pptx primitives.
...
def main() -> int:
    ...
    from imagegen_prompt import main as write_imagegen_prompt

    sys.argv = [
        str(Path(__file__).with_name("imagegen_prompt.py")),
        prompt,
        "--quality",
        "medium",
        "--force",
