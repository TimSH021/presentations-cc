...
Use python-pptx and headless package tooling only. If those cannot inspect,
...
```bash
python "$SKILL_DIR/scripts/prepare_starter_deck.py" \
  --pptx "<source.pptx>" \
...
Export the final deck, render final slide previews, and review each adapted
output slide against its mapped source slide render.
